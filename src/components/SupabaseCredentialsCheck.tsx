...
function setCookiesForSupabase(url: string, key: string) {
  ...
  console.log('Supabase credentials saved to cookies and will be available to server components via headers');
}

// Function to clear Supabase cookies
function clearSupabaseCookies() {
  // Set cookies with expired date to remove them
  document.cookie = `supabase-url=; path=/; expires=Thu, 01 Jan 1970 00:00:01 GMT`;
  document.cookie = `supabase-key=; path=/; expires=Thu, 01 Jan 1970 00:00:01 GMT`;
}
...
export default function SupabaseCredentialsCheck() {
  ...
  useEffect(() => {
    // Check if we're on the settings page - don't show the modal there
    ...
    const noCredentials = !hasSupabaseCredentials();
    
    // If we're on the home page (root path) or it's the first visit or no credentials exist,
    // show the modal as an entry point to the application
    if (
      (typeof window !== 'undefined' && window.location.pathname === '/') || 
      firstVisit || 
      (noCredentials && !localStorage.getItem('userSkippedCredentials'))
    ) {
      setShowModal(true);
